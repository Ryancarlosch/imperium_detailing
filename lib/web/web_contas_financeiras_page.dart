...
class _WebExtratoContaPageState extends State<WebExtratoContaPage> {
  ...
  Future<void> _conciliar() async {
    ...
    var data = _mes.year == agora.year && _mes.month == agora.month
        ? DateTime(agora.year, agora.month, agora.day)
        : DateTime(_mes.year, _mes.month + 1, 0);
    final saldo = TextEditingController(
      text: comparativo.atual.saldoFinalMes
          .toStringAsFixed(2)
          .replaceAll('.', ','),
    );
    final observacoes = TextEditingController();
    var criarAjuste = false;

    final confirmar = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setLocal) => AlertDialog(
          title: const Text('Conciliar saldo da conta'),
          content: SizedBox(
            width: 600,
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Saldo informado no banco / caixa',
                      prefixText: 'R\$ ',
                    ),
                  ),
                  const SizedBox(height: 12),
                  SwitchListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('Criar ajuste se houver diferença'),
                    subtitle: const Text(
                      'O ajuste usa Correção de caixa (9.05) e não impacta a DRE.',
                    ),
                    value: criarAjuste,
                    onChanged: widget.conta.ativa
                        ? (value) => setLocal(() => criarAjuste = value)
                        : null,
                  ),
                  if (!widget.conta.ativa)
                    const Padding(
                      padding: EdgeInsets.only(bottom: 8),
                      child: Text(
                        'A conta está inativa. É possível conferir o saldo, mas não criar ajuste.',
    ...
    try {
      ...
      final resultado = await _service.registrarConciliacao(
        contaId: widget.conta.id,
        data: data,
        saldoInformado: _numero(saldo.text),
        criarAjuste: criarAjuste,
        observacoes: observacoes.text,
      );
      await _carregar();
      if (!mounted) return;
      final status = (resultado['status'] ?? 'Conciliado').toString();
      final diferenca = _doubleValor(resultado['diferenca']);
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(
          SnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Conciliar saldo',
            onPressed: _carregando ? null : _conciliar,
            icon: const Icon(Icons.verified_outlined),
          ),
          IconButton(
          ...
          children: [
            ...
            if (atual.conciliacoes.isEmpty)
              const _EstadoVazio(
                ...
                detalhe:
                    'Use “Conciliar saldo” para registrar a conferência desta conta.',
              )
            else
