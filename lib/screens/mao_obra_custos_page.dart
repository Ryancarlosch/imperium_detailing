...
class _MaoObraCustosPageState extends State<MaoObraCustosPage> {
  ...
  Future<void> _abrirFormulario([ColaboradorCusto? colaborador]) async {
    final resultado = await showModalBottomSheet<bool>(
      ...
      builder: (_) =>
          _ColaboradorForm(repository: _repository, colaborador: colaborador),
    );
    if (resultado == true) {
      await _carregar();
    }
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  const Text(
                    'Cadastre também seu próprio pró-labore quando sua mão de obra participa dos serviços. Isso evita considerar seu tempo como custo zero.',
                        ...
                        child: ListTile(
                          ...
                          subtitle: Text(
                            [
                              ...
                              '${item.horasProdutivasMes.toStringAsFixed(1)} h produtivas/mês',
                              '${_moeda.format(item.custoHoraProdutiva)}/h',
                            ].join(' • '),
                          ),
...
class _ColaboradorForm extends StatefulWidget {
  const _ColaboradorForm({required this.repository, this.colaborador});
...
class _ColaboradorFormState extends State<_ColaboradorForm> {
  ...
  void initState() {
    ...
    _horas = TextEditingController(
      text: item == null
          ? ''
          : item.horasProdutivasMes.toStringAsFixed(1).replaceAll('.', ','),
    );
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Horas produtivas por mês',
                  suffixText: 'h',
              ...
              const Text(
                'Horas produtivas são as horas efetivamente disponíveis para executar serviços faturáveis, não a carga horária total.',
