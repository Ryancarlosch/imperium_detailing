...
class _ItemEstoqueDetalhesPageState extends State<ItemEstoqueDetalhesPage> {
  ...
  Future<void> _carregar() async {
    final item = await _repository.buscarItemPorId(widget.itemId);
  ...
  Future<void> _alterarQuantidade(bool entrada) async {
    final item = _item;
    ...
    final quantidade = await showDialog<double>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          title: Text(entrada ? 'Adicionar estoque' : 'Retirar estoque'),
          content: TextField(
            autofocus: true,
            ...
            decoration: InputDecoration(
              labelText: 'Quantidade (${item.unidade})',
            ),
          ),
          actions: [
            TextButton(
    ...
    if (!entrada && quantidade > item.quantidade) {
      if (!mounted) return;

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('A retirada é maior que o estoque disponível.'),
        ),
      );
      return;
    }
    ...
    await _repository.registrarMovimentacao(
      MovimentacaoEstoque(
        ...
        observacao: entrada
            ? 'Entrada manual pela tela de detalhes do item'
            : 'Saída manual pela tela de detalhes do item',
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => const [
              PopupMenuItem(value: 'excluir', child: Text('Excluir item')),
            ],
          ),
        ],
      ),
                    ...
                    child: Column(
                      children: [
                        ...
                        _Linha(
                          ...
                          valor: _moeda.format(item.custoUnitario),
                        ),
                        _Linha(
                          titulo: 'Valor em estoque',
                          valor: _moeda.format(item.valorTotal),
                        ),
                        _Linha(
                          titulo: 'Fornecedor',
