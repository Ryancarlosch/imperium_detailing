...
class _NovoOrcamentoPageState
  ...
  final _formatoMoeda = NumberFormat.currency(
    locale: 'pt_BR',
    symbol: 'R\$',
  );
  ...
  DateTime _validade = DateTime.now().add(
  ...
  void initState() {
    ...
    if (orcamento != null) {
      ...
      if (orcamento.itens.isNotEmpty) {
        _itens = List<ItemOrcamento>.from(
          orcamento.itens,
        );
      } else if (
      orcamento.servico.trim().isNotEmpty ||
          orcamento.valor > 0) {
        _itens = [
          ItemOrcamento(
            servico:
            orcamento.servico.trim().isEmpty
                ? 'Serviço'
                : orcamento.servico,
  ...
  double _converterValor(
      String texto,
      ) {
    final valorLimpo = texto
        .trim()
        .replaceAll('R\$', '')
        .replaceAll(' ', '');
  ...
  Future<void> _abrirFormularioItem({
    ...
    final resultado =
    await showModalBottomSheet<ItemOrcamento>(
      context: context,
    ...
    setState(() {
      if (indice == null) {
        ...
        _itens[indice] = resultado.copyWith(
          ordem: indice,
        );
      }
    });
  ...
  Future<void> _removerItem(
    ...
    setState(() {
      ...
      _itens = List<ItemOrcamento>.generate(
        _itens.length,
            (index) {
          return _itens[index].copyWith(
            ordem: index,
          );
        },
      );
  ...
  Future<void> _salvar() async {
    ...
    try {
      ...
      final orcamento = Orcamento(
        ...
        validade:
        _validade.toIso8601String(),
        status: _status,
        observacoes:
        _observacoesController.text.trim(),
        desconto: _desconto,
        itens: List<ItemOrcamento>.generate(
          _itens.length,
              (index) {
            return _itens[index].copyWith(
              ordem: index,
...
class _FormularioItemOrcamento
    extends StatefulWidget {
  const _FormularioItemOrcamento({
    required this.ordem,
    this.item,
  });
  ...
  @override
  State<_FormularioItemOrcamento>
  createState() =>
      _FormularioItemOrcamentoState();
}

class _FormularioItemOrcamentoState
    extends State<_FormularioItemOrcamento> {
  final _formKey = GlobalKey<FormState>();
...
class _EstadoSemItens
    extends StatelessWidget {
  const _EstadoSemItens({
