...
class _OrcamentoDetalhesPageState extends State<OrcamentoDetalhesPage> {
  ...
  Future<void> _carregar() async {
    try {
      ...
      _mensagem('Não foi possível carregar o orçamento: $erro', erro: true);
    }
  }

  List<ItemOrcamento> _extrairItens(dynamic valor) {
    if (valor is! List) {
      return [];
    }
    ...
    return valor
        .whereType<Map>()
        .map((item) => ItemOrcamento.fromMap(Map<String, dynamic>.from(item)))
        .toList();
  }

  Future<void> _editar() async {
    final orcamento = await _repository.buscarPorId(widget.orcamentoId);
  ...
  Future<void> _alterarStatus(String novoStatus) async {
    ...
    try {
      await _repository.atualizarStatus(widget.orcamentoId, novoStatus);
      ...
      _mensagem('Status atualizado para $novoStatus.');
    } catch (erro) {
      if (!mounted) {
        return;
      }

      _mensagem('Não foi possível alterar o status: $erro', erro: true);
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _gerarOrdemServico() async {
    ...
    try {
      final criou = await Navigator.push<bool>(
        context,
        MaterialPageRoute(
          builder: (_) => NovaOrdemServicoPage(orcamentoId: widget.orcamentoId),
        ),
      );
      ...
      if (criou == true) {
        ...
        _mensagem('Ordem de Serviço vinculada ao orçamento.');
      }
    } catch (erro) {
      if (!mounted) {
        return;
      ...
      _mensagem('Não foi possível abrir a Ordem de Serviço: $erro', erro: true);
    } finally {
      if (mounted) {
        setState(() {
          _abrindoOrdem = false;
  ...
  Future<void> _pdf({required bool recibo, required bool compartilhar}) async {
    final dados = _dados;
    ...
    try {
      if (compartilhar) {
        await _pdfService.compartilhar(dados, recibo: recibo);
      } else {
        await _pdfService.visualizar(dados, recibo: recibo);
      }
    } catch (erro) {
      if (!mounted) {
        return;
      ...
      _mensagem('Não foi possível gerar o PDF: $erro', erro: true);
    } finally {
      if (mounted) {
        setState(() {
          _gerandoPdf = false;
  ...
  Future<void> _excluir() async {
    ...
    try {
      await _repository.excluirOrcamento(widget.orcamentoId);
      ...
      _mensagem('Não foi possível excluir o orçamento: $erro', erro: true);
    }
  }
  ...
  double _valorTotalCalculado() {
    final subtotal = _numero('subtotal_itens') > 0
        ? _numero('subtotal_itens')
        : _itens.fold<double>(0, (total, item) => total + item.subtotal);
    ...
    return (subtotal - desconto).clamp(0, double.infinity);
  }
  ...
  Future<void> _executarAcaoWhatsApp(
    ...
    final telefone = _texto('cliente_telefone');
    final cliente = _texto('cliente_nome');
    final observacoes = _texto('observacoes');

    if (telefone.isEmpty) {
      _mensagem('O cliente não possui telefone cadastrado.', erro: true);
      return;
    }
    ...
    try {
      await acao(
        telefone: telefone,
        cliente: cliente.isEmpty ? 'Cliente' : cliente,
        numero: 'Orçamento #${widget.orcamentoId}',
        valor: _moeda.format(_valorTotalCalculado()),
      ...
      _mensagem('Não foi possível abrir o WhatsApp: $erro', erro: true);
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _enviarOrcamentoWhatsApp() async {
    await _executarAcaoWhatsApp(({
      required String telefone,
      required String cliente,
      required String numero,
      required String valor,
      ...
      return WhatsAppService.enviarOrcamento(
        telefone: telefone,
        cliente: cliente,
  ...
  Future<void> _enviarAprovacaoWhatsApp() async {
    await _executarAcaoWhatsApp(({
      required String telefone,
      required String cliente,
      required String numero,
      ...
      return WhatsAppService.enviarAprovacaoOrcamento(
        telefone: telefone,
        cliente: cliente,
        numeroOrcamento: numero,
        valorTotal: valor,
  ...
  Future<void> _abrirOpcoesWhatsApp() async {
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                onTap: () {
                  Navigator.of(bottomContext).pop('orcamento');
                },
              ),
              ListTile(
                leading: const CircleAvatar(
                ...
                onTap: () {
                  Navigator.of(bottomContext).pop('aprovacao');
                },
              ),
              ListTile(
                leading: const CircleAvatar(
    ...
    if (opcao == 'orcamento') {
      await _enviarOrcamentoWhatsApp();
      return;
    }

    if (opcao == 'aprovacao') {
      await _enviarAprovacaoWhatsApp();
      return;
    }
  ...
  String _texto(String chave) {
    return (_dados?[chave] ?? '').toString().trim();
  }
  ...
  String _dataTexto(String chave) {
    final valor = DateTime.tryParse(_texto(chave));
  ...
  Widget build(BuildContext context) {
    final dados = _dados;
    final status = _texto('status').isEmpty ? 'Pendente' : _texto('status');
    ...
    final total = totalBanco > 0
        ? totalBanco
        : (subtotal - desconto).clamp(0, double.infinity);
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: dados == null || _excluindo ? null : _editar,
              icon: const Icon(Icons.edit_outlined),
            ),
            PopupMenuButton<String>(
              enabled: dados != null && !_excluindo,
              onSelected: (opcao) {
                if (opcao == 'excluir') {
                  _excluir();
                }
                ...
                child: ListView(
                  ...
                  children: [
                    _CabecalhoOrcamento(
                      ...
                      status: status,
                      corStatus: _corStatus(status),
                      iconeStatus: _iconeStatus(status),
                    ...
                    _Secao(
                      ...
                      child: Column(
                        children: [
                          _LinhaInformacao(
                            titulo: 'Cliente',
                            valor: _texto('cliente_nome').isEmpty
                                ? 'Não informado'
                                : _texto('cliente_nome'),
                            icone: Icons.person_outline,
                          ),
                          ...
                          _LinhaInformacao(
                            titulo: 'Telefone',
                            valor: _texto('cliente_telefone').isEmpty
                                ? 'Não informado'
                                : _texto('cliente_telefone'),
                              ...
                              children: List.generate(_itens.length, (indice) {
                                ...
                                return Padding(
                                  ...
                                  child: _CardServico(
                                    ...
                                    valorUnitario: _moeda.format(
                                      item.valorUnitario,
                                    ),
                                    subtotal: _moeda.format(item.subtotal),
                                  ),
                                );
                              }),
                            ),
                    ...
                    _Secao(
                      ...
                      child: Column(
                        children: [
                          _LinhaInformacao(
                            titulo: 'Emissão',
                            valor: _dataTexto('data_emissao'),
                            icone: Icons.calendar_today_outlined,
                          ),
                          const Divider(height: 24),
                          _LinhaInformacao(
                            titulo: 'Validade',
                            valor: _dataTexto('validade'),
                            icone: Icons.event_available_outlined,
                          ),
                          if (_texto('observacoes').isNotEmpty) ...[
                            const Divider(height: 24),
                            _TextoLongo(
                              titulo: 'Observações',
                              valor: _texto('observacoes'),
                            ),
                          ],
                        ],
                      ),
                    ...
                    FilledButton.icon(
                      onPressed: _gerandoPdf
                          ? null
                          : () {
                              _pdf(recibo: false, compartilhar: false);
                            },
                      icon: _gerandoPdf
                          ? const SizedBox(
                              width: 20,
                    ...
                    OutlinedButton.icon(
                      onPressed: _gerandoPdf
                          ? null
                          : () {
                              _pdf(recibo: false, compartilhar: true);
                            },
                      icon: const Icon(Icons.share_outlined),
                      label: const Text('Compartilhar orçamento'),
                    ),
                    const SizedBox(height: 10),
                    OutlinedButton.icon(
                    ...
                    if (status == 'Aprovado') ...[
                      ...
                      FilledButton.icon(
                        onPressed: _gerandoPdf
                            ? null
                            : () {
                                _pdf(recibo: true, compartilhar: true);
                              },
                        icon: const Icon(Icons.receipt_long_outlined),
                        label: const Text('Gerar e compartilhar recibo'),
                      ),
                    ],
...
class _CabecalhoOrcamento extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: const Color(0xFFD6A84B).withValues(alpha: 0.15),
                ),
                child: Text(
                  numero.toString(),
          ...
          Wrap(
            ...
            runSpacing: 10,
            children: [
              _InformacaoServico(titulo: 'Quantidade', valor: quantidade),
              _InformacaoServico(
...
class _InformacaoServico extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            valor,
            style: TextStyle(
              color: destaque ? const Color(0xFFD6A84B) : null,
              fontWeight: FontWeight.bold,
            ),
          ),
        ],
...
class _TextoLongo extends StatelessWidget {
  const _TextoLongo({required this.titulo, required this.valor});
