...
class ProdutoOrdemServicoRepository {
  ...
  Future<List<ProdutoOrdemServico>>
  listarProdutosPorOrdemServico(
    ...
    return resultado
        .map(ProdutoOrdemServico.fromMap)
        .toList();
  }

  Future<ProdutoOrdemServico?>
  buscarProdutoPorId(
      int id,
      ) async {
    final database =
    ...
    return ProdutoOrdemServico.fromMap(
      resultado.first,
    );
  }

  Future<ProdutoOrdemServico?>
  buscarProdutoNaOrdem({
    ...
    final resultado = await database.query(
      'ordem_servico_produtos',
      where:
      'ordem_servico_id = ? AND produto_id = ?',
      whereArgs: [
        ordemServicoId,
        produtoId,
      ],
  ...
  Future<int> atualizarProduto(
    ...
    if (produto.id == null) {
      throw ArgumentError(
        'Não é possível atualizar um produto sem ID.',
      );
    }
  ...
  Future<int> adicionarOuSomarProduto(
      ProdutoOrdemServico produto,
      ) async {
  ...
  Future<int> excluirProdutosDaOrdem(
      int ordemServicoId,
      ) async {
    final database =
    await _appDatabase.database;
    ...
    final produtos =
    await listarProdutosPorOrdemServico(
      ordemServicoId,
    );

    final possuiBaixado = produtos.any(
          (produto) => produto.baixadoEstoque,
    );

    if (possuiBaixado) {
      throw Exception(
        'Existem produtos que já tiveram baixa no estoque.',
      );
    }
  ...
  Future<double> calcularCustoTotal(
      int ordemServicoId,
      ) async {
    final produtos =
    await listarProdutosPorOrdemServico(
  ...
  Future<bool> possuiProdutoBaixado(
      int ordemServicoId,
      ) async {
    final database =
    await _appDatabase.database;
    ...
    final resultado = await database.query(
      ...
      columns: ['id'],
      where:
      'ordem_servico_id = ? AND baixado_estoque = 1',
      whereArgs: [ordemServicoId],
      limit: 1,
    );
  ...
  Future<int> marcarComoNaoBaixado(
      int id,
      ) async {
    final database =
    await _appDatabase.database;
