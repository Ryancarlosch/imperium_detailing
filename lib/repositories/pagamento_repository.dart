...
class PagamentoRepository {
    ...
    final resumoPagamentos = await database.rawQuery(
      ...
      SELECT
        COALESCE(SUM(CASE
          ...
          ELSE 0
        END), 0) AS taxas_registradas,
        COALESCE(
          SUM(CASE WHEN p.status = 'Pago' THEN p.valor ELSE 0 END),
          0
        ) - COALESCE(SUM(CASE
          WHEN p.status = 'Pago' THEN p.taxa_operacao
          WHEN p.status = 'Estornado'
  ...
  Future<void> corrigirPagamentoLancadoPorEngano({
    required int pagamentoId,
    required String motivo,
  }) async {
    final motivoLimpo = motivo.trim();
    ...
    if (motivoLimpo.length < 5) {
      throw ArgumentError(
        'Informe um motivo da correção com pelo menos 5 caracteres.',
      );
    }
    ...
    await database.transaction((transaction) async {
      ...
      if ((pagamento['status'] ?? '').toString() != 'Pago') {
        throw StateError(
          'Somente pagamentos confirmados podem ser corrigidos.',
        );
      }
      ...
      // Se a maquininha tinha repasse automático ao cliente, esse acréscimo
      // pertence ao pagamento que estamos corrigindo. Como o recebimento foi
      // lançado por engano, o repasse também nunca deveria ter existido.
      ...
      await transaction.update(
        'ordem_servico_pagamentos',
        {
          'status': 'Estornado',
          'estornado_em': agora,
          'motivo_estorno': 'Correção: $motivoLimpo',
          'atualizado_em': agora,
        },
        where: 'id = ?',
        whereArgs: [pagamentoId],
      ...
      // O dinheiro nunca aconteceu. Em vez de criar uma saída, cancelamos
      // os movimentos automáticos do recebimento e da taxa, preservando-os
      // para auditoria.
      await transaction.update(
        'movimentos_financeiros',
        {
          'status': 'Cancelado',
          'observacoes': 'Correção de recebimento: $motivoLimpo',
        },
        where: "pagamento_id = ? AND status = 'Realizado'",
        whereArgs: [pagamentoId],
      );
      ...
      if (ajustesRepasse.isNotEmpty) {
        await transaction.update(
          'ordem_servico_ajustes_financeiros',
          {
            ...
            'cancelado_em': agora,
            'motivo_cancelamento':
                'Cancelado automaticamente pela correção do recebimento.',
          },
          where:
              "pagamento_id = ? AND status = 'Ativo' AND origem = 'Taxa de maquininha'",
          whereArgs: [pagamentoId],
      ...
      if (parcelaNumero != null && totalParcelas != null) {
        final valorBaseParcela = (valor - repasseCliente)
            .clamp(0, double.infinity)
            .toDouble();
  ...
  Future<void> devolverPagamentoAoCliente({
    required int pagamentoId,
    required String motivo,
  }) {
    ...
    await database.transaction((transaction) async {
      ...
      await transaction.update(
        'ordem_servico_pagamentos',
        {
          'status': 'Estornado',
          'estornado_em': agora,
          'motivo_estorno': 'Devolução: $motivoLimpo',
          'atualizado_em': agora,
        },
        where: 'id = ?',
        whereArgs: [pagamentoId],
      ...
      await transaction.insert('movimentos_financeiros', {
        'tipo': 'saída',
        'descricao': 'Devolução ao cliente - OS ${ordem['numero']}',
        'valor': valor,
