...
class ResumoFinanceiroPdf {
  ...
  static pw.Widget criar({
    ...
    final totalCalculado = subtotal - desconto + acrescimo;

    final total = totalInformado ?? (totalCalculado < 0 ? 0 : totalCalculado);
  ...
  static pw.Widget criarPorMapa({
    ...
    final subtotal = _primeiroNumero(dados, [
      'subtotal',
      'valor_subtotal',
      'total_servicos',
    ]);

    final desconto = _primeiroNumero(dados, ['desconto', 'valor_desconto']);

    final acrescimo = _primeiroNumero(dados, [
      'acrescimo',
      'valor_acrescimo',
      'taxa',
    ]);

    final total = _primeiroNumeroNullable(dados, [
      'total',
      'valor_total',
      'valor_final',
    ]);

    final valorPago = _primeiroNumero(dados, ['valor_pago', 'pago', 'entrada']);

    final formaPagamento = _primeiroTexto(dados, [
      'forma_pagamento',
      'pagamento',
      'metodo_pagamento',
    ]);

    final condicaoPagamento = _primeiroTexto(dados, [
      'condicao_pagamento',
      'parcelamento',
  ...
  static pw.Widget _criarDadosPagamento({
    ...
    final possuiDados =
        formaPagamento.trim().isNotEmpty || condicaoPagamento.trim().isNotEmpty;
    ...
    return pw.Column(
      ...
      children: [
        ...
        if (formaPagamento.trim().isNotEmpty &&
            condicaoPagamento.trim().isNotEmpty)
          pw.SizedBox(height: 11),
        if (condicaoPagamento.trim().isNotEmpty)
          _criarCampoPagamento(
  ...
  static pw.Widget _criarValores({
    ...
    return pw.Column(
      children: [
        _criarLinhaValor(rotulo: 'Subtotal', valor: subtotal),
        if (desconto > 0) ...[
        ...
        _criarLinhaTotal(contexto: contexto, total: total),
        if (valorPago > 0) ...[
          pw.SizedBox(height: 9),
          _criarLinhaValor(
            rotulo: 'Valor pago',
        ...
        if (mostrarSaldo) ...[
          pw.SizedBox(height: 7),
          _criarLinhaValor(
            rotulo: saldo > 0 ? 'Saldo pendente' : 'Saldo',
            valor: saldo < 0 ? 0 : saldo,
            destaque: true,
            corValor: saldo > 0 ? PdfColors.red700 : PdfColors.green700,
          ),
        ],
      ],
    );
    ...
    return pw.Row(
      children: [
        ...
        pw.Text(
          ...
          style: pw.TextStyle(
            fontSize: 8.5,
            color: corValor ?? PdfColors.grey900,
            fontWeight: destaque ? pw.FontWeight.bold : pw.FontWeight.normal,
          ),
        ),
      ],
  ...
  static double _primeiroNumero(
    ...
    return _primeiroNumeroNullable(mapa, chaves) ?? 0;
  }
