...
void main() {
  ...
  group('ImperiumApp', () {
    ...
    testWidgets(
      'mantém locale pt-BR e tema escuro',
          (tester) async {
        _configurarTela(tester);
  ...
  group('Assistente de primeiro uso', () {
    testWidgets(
      'abre, avança e fecha corretamente',
          (tester) async {
        _configurarTela(tester);

        await tester.pumpWidget(
          const MaterialApp(
            home: _AssistenteTesteHost(),
          ),
        );

        await tester.tap(
          find.text('Abrir assistente'),
        );
        ...
        expect(
          find.text('Configurações da empresa'),
          findsOneWidget,
        );
...
Future<void> _removerBancoDeTeste(
    String caminhoBanco,
    ) async {
  await AppDatabase.instance.fecharBanco();
