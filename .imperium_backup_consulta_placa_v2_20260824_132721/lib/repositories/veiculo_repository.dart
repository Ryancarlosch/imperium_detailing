...
class VeiculoRepository {
  ...
  // veiculo-insercao-idempotente-v1
  final Map<String, Future<int>> _insercoesEmAndamento =
      <String, Future<int>>{};
  ...
  Future<int> inserirVeiculo(Veiculo veiculo) async {
    final chave = _chaveInsercao(veiculo);
    final existente = _insercoesEmAndamento[chave];

    if (existente != null) return existente;

    final operacao = _inserirVeiculoUnico(veiculo);
    _insercoesEmAndamento[chave] = operacao;

    try {
      return await operacao;
    } finally {
      if (identical(_insercoesEmAndamento[chave], operacao)) {
        _insercoesEmAndamento.remove(chave);
      }
    }
  }

  Future<int> _inserirVeiculoUnico(Veiculo veiculo) async {
    final database = await _appDatabase.database;
    final placa = _normalizarPlaca(veiculo.placa);

    if (placa.isNotEmpty) {
      final repetido = await database.rawQuery(
        '''
        SELECT id
        FROM veiculos
        WHERE cliente_id = ?
        ...
        LIMIT 1
        ''',
        [veiculo.clienteId, placa],
      );

      if (repetido.isNotEmpty) {
        final id = _int(repetido.first['id']);
        if (id > 0) {
          await _sincronizar();
          return id;
        }
  ...
  String _chaveInsercao(Veiculo veiculo) {
    return <String>[
      veiculo.clienteId.toString(),
      veiculo.marca.trim().toLowerCase(),
      veiculo.modelo.trim().toLowerCase(),
      _normalizarPlaca(veiculo.placa),
      veiculo.cor.trim().toLowerCase(),
      veiculo.ano.trim().toLowerCase(),
      veiculo.observacoes.trim().toLowerCase(),
    ].join('|');
  }

  String _normalizarPlaca(String placa) {
    return placa.trim().toUpperCase().replaceAll('-', '').replaceAll(' ', '');
  }
